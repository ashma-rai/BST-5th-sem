...
void welcomeDisplay();//display welcome at the beginning of program
...
{   //this function invoked only when menu options insert or search is selected
    if(welcomeKeyPressed)
    {
        if(key==13)
        {
            glutDisplayFunc(display);
            menuDetails();
            welcomeKeyPressed=false;
        }
        glutPostRedisplay();//change control to display
    }
    if(inputKeyboardVal)
    {
